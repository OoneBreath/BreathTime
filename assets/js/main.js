...
function initGeolocation() {
    if ("geolocation" in navigator) {
        ...
        handleGeolocationError(new Error('Geolokalizacja nie jest wspierana'));
    }
}

// Obsługa błędów geolokalizacji
async function handleGeolocationError(error) {
    console.log('Próba lokalizacji przez IP...');
    try {
        ...
        if (data && data.latitude && data.longitude) {
            ...
            console.log('Lokalizacja przez IP:', data.city);
        } else {
            throw new Error('Brak danych lokalizacji z IP');
        }
    } catch (error) {
        console.log('Nie udało się pobrać lokalizacji przez IP, używam domyślnej');
        userLatitude = 52.237049;
        userLongitude = 21.017532; // Warszawa jako ostateczność
        getUserTimezone(userLatitude, userLongitude);
    }
}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Inicjalizacja geolokalizacji dla zmiany tekstury globusa
    initGeolocation();
